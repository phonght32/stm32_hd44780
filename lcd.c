...
stm_err_t _write_cmd_4bit(lcd_pin_t pin, uint8_t data) {
...
lcd_handle_t lcd_init(lcd_cfg_t *config) {
	...
	LCD_CHECK(!_write_func(config->pin, 0x02), LCD_INIT_ERR_STR, {_lcd_cleanup(handle); return NULL;});
	vTaskDelay(LCD_TICK_DELAY_DEFAULT / portTICK_PERIOD_MS);

	LCD_CHECK(!_write_func(config->pin, 0x28), LCD_INIT_ERR_STR, {_lcd_cleanup(handle); return NULL;});
	vTaskDelay(LCD_TICK_DELAY_DEFAULT / portTICK_PERIOD_MS);

	LCD_CHECK(!_write_func(config->pin, 0x06), LCD_INIT_ERR_STR, {_lcd_cleanup(handle); return NULL;});
	vTaskDelay(LCD_TICK_DELAY_DEFAULT / portTICK_PERIOD_MS);

	LCD_CHECK(!_write_func(config->pin, 0x0C), LCD_INIT_ERR_STR, {_lcd_cleanup(handle); return NULL;});
	vTaskDelay(LCD_TICK_DELAY_DEFAULT / portTICK_PERIOD_MS);

	LCD_CHECK(!_write_func(config->pin, 0x01), LCD_INIT_ERR_STR, {_lcd_cleanup(handle); return NULL;});
	vTaskDelay(LCD_TICK_DELAY_DEFAULT / portTICK_PERIOD_MS);
	...
	handle->_write_func = _write_func;
...
stm_err_t lcd_clear(lcd_handle_t handle) {
	LCD_CHECK(!gpio_set_level(handle->pin.gpio_port_rs, handle->pin.gpio_num_rs, false), LCD_CLEAR_ERR_STR, return STM_FAIL);
	handle->_write_func(handle->pin, 0x01);
...
stm_err_t lcd_home(lcd_handle_t handle) {
	LCD_CHECK(!gpio_set_level(handle->pin.gpio_port_rs, handle->pin.gpio_num_rs, false), LCD_CLEAR_ERR_STR, return STM_FAIL);
	handle->_write_func(handle->pin, 0x02);
	vTaskDelay(2 / portTICK_PERIOD_MS);
...
stm_err_t lcd_write_string(lcd_handle_t handle, uint8_t *str) {
	LCD_CHECK(!gpio_set_level(handle->pin.gpio_port_rs, handle->pin.gpio_num_rs, true), LCD_CLEAR_ERR_STR, return STM_FAIL);
	while (*str) {
		handle->_write_func(handle->pin, *str);
		str++;
	}
